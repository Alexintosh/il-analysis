...
async function main() {
    ...
    const query = gql`
        {
            liquidityPositionSnapshots(first: 1000, where: { user: "${user}"} ) {
                timestamp
                reserveUSD
    ...
    // console.log(result)
    const ethPrice = result.bundle.ethPrice
    // console.log(ethPrice)

    for (let snapshot of result['liquidityPositionSnapshots']) {
        ...
        let metrics = getMetricsForPositionWindow(snapshot, currentPosition)
        console.log(metrics)
    }
}
